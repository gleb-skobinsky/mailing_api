...
class MailingSerializer(serializers.Serializer):
    ...
    mobile_code = serializers.IntegerField(
        required=False, allow_null=True, min_value=100, max_value=999
    )
...
class ClientSerializer(serializers.Serializer):
    ...
    phone_number = serializers.RegexField(
        ...
        validators=[
            UniqueValidator(
                queryset=Client.objects.all(), message=("Phone number already exists")
            )
        ],
    )
